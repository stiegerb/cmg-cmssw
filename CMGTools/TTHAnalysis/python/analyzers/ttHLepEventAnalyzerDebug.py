...
class ttHLepEventAnalyzerDebug( Analyzer ):
    def __init__(self, cfg_ana, cfg_comp, looperName ):
        super(ttHLepEventAnalyzerDebug,self).__init__(cfg_ana,cfg_comp,looperName)
    ...
    def declareHandles(self):
        super(ttHLepEventAnalyzerDebug, self).declareHandles()
    ...
    def beginLoop(self):
        super(ttHLepEventAnalyzerDebug,self).beginLoop()
    ...
    def process(self, iEvent, event):
        ...
        #########################################################
        ### DEBUGGING OUTPUT ####################################
        print '%6s, %6s, %6s, %6s, %9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s' % ('pdgId', 'pt', 'eta', 'phi', 'mcMatchId', 'mcMtchAny', 'sip3d', 'neuRelIso', 'chRelIso', 'jetDR', 'jetPtRat', 'jetBTag','log|dxy|', 'log|dz|', 'mvaID', 'innerHits', 'mva')
        for lep in event.selectedLeptons:
            if lep.pt() < 4.5: continue
            output = '%6d, %6.2f, %6.2f, %6.2f, %9d, %9d, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9d, %9.3f'
            values = (lep.pdgId(), lep.pt(), lep.eta(), lep.phi(),
                lep.mcMatchId, lep.mcMatchAny,
                # lep.sip3D(),
                ROOT.scaleSip3dMC(lep.sip3D(), lep.pdgId(), lep.pt(), lep.eta(), lep.mcMatchId, lep.mcMatchAny),
                lep.relIso(dBetaFactor=0.5) - lep.chargedHadronIso()/lep.pt(),
                lep.chargedHadronIso()/lep.pt(),
                # min(deltaR(lep.eta(),lep.phi(),lep.jet.eta(),lep.jet.phi()),0.5),
                ROOT.correctJetDRMC(min(deltaR(lep.eta(),lep.phi(),lep.jet.eta(),lep.jet.phi()),0.5), lep.pdgId(), lep.pt(), lep.eta(), lep.mcMatchId, lep.mcMatchAny),
                # min(lep.pt()/lep.jet.pt(),1.5),
                ROOT.correctJetPtRatioMC(min(lep.pt()/lep.jet.pt(),1.5), lep.pdgId(), lep.pt(), lep.eta(), lep.mcMatchId, lep.mcMatchAny),
                max((lep.jet.btag('combinedSecondaryVertexBJetTags') if hasattr(lep.jet, 'btag') else -99) ,0.),
                # log(abs(lep.dxy())),
                ROOT.scaleDxyMC(log(abs(lep.dxy())), lep.pdgId(), lep.pt(), lep.eta(), lep.mcMatchId, lep.mcMatchAny),
                # log(abs(lep.dz())),
                ROOT.scaleDzMC(log(abs(lep.dz())), lep.pdgId(), lep.pt(), lep.eta(), lep.mcMatchId, lep.mcMatchAny),
                lep.mvaNonTrigV0() if abs(lep.pdgId())==11 else -1,
                lep.numberOfHits() if abs(lep.pdgId())==11 else -1,
                # lep.mvaNoCorr)
                lep.mvaValue)
            print output % values
            # print lep.chargedHadronIso()
            # print lep.jet.pt()
